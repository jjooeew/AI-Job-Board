...
CREATE TYPE "public"."job_listings_type" AS ENUM('casual', 'internship', 'part-time', 'full-time');--> statement-breakpoint
...
CREATE TABLE "organisation_user_settings" (
	...
	"createdAt" timestamp with time zone DEFAULT now() NOT NULL,
	"updatedAt" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "organisation_user_settings_userId_organisationId_pk" PRIMARY KEY("userId","organisationId")
);
--> statement-breakpoint
...
ALTER TABLE "organisation_user_settings" ADD CONSTRAINT "organisation_user_settings_organisationId_organisations_id_fk" FOREIGN KEY ("organisationId") REFERENCES "public"."organisations"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
